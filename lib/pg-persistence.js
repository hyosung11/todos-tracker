...
module.exports = class PgPersistence {
  ...
  /* Returns a promise that resolves to `true` if `username` and `password` combine to identify a legitimate application user, `false` if either `username` or `password` is invalid. */
  async authenticate(username, password) {
    const AUTHENTICATE =
      "SELECT null FROM users WHERE username = $1 AND password = $2";

    let result = await dbQuery(AUTHENTICATE, username, password);
    return result.rowCount > 0;
  }

  /* Mark all todos in the specified todo list as done. Returns a promise that resolves to `true` on success, `false` if the todo list doesn't exist. The todo list ID must be numeric. */
  ...
  todo lists. Returns a promise that resolves to `true` on success, `false` if the todo list already exists. */
  async createTodoList(title) {
    const CREATE_TODOLIST =
      "INSERT INTO todolists (title) VALUES ($1)";
  ...
  /* Delete the specified todo from the specified todo list. Returns a promise that resolves to `true` on success, `false` if the todo or todo list doesn't exist. */
  async deleteTodo(todoListId, todoId) {
    const DELETE_TODO =
      "DELETE FROM todos WHERE todolist_id = $1 AND id = $2";
  ...
  /* Returns a promise that resolves to `true` if a todo list with the specified title exists in the list of todo lists, `false` otherwise. */
  async existsTodoListTitle(title) {
    const FIND_TODOLIST =
      "SELECT null FROM todolists WHERE title = $1";
  ...
  /* Returns `true` if `error` seems to indicate a `UNIQUE` constraint violation, `false` otherwise. */
  isUniqueConstraintViolation(error) {
    return /duplicate key value violates unique constraint/.test(String(error));
  }

  /* Returns a copy of the indicated todo in the indicated todo list. Returns `undefined` if either the todo list or the todo is not found. Note that both IDs must be numeric. */
  ...
  /* Set a new title for the specified todo list. Returns a promise that resolves to  `true` on success, `false` if the todo list isn't found. */
  async setTodoListTitle(todoListId, title) {
    const UPDATE_TITLE =
      "UPDATE todolists SET title = $1 WHERE id = $2";
  ...
  /* Toggle a todo between the done and not done state. Returns a promise that resolves to `true` on success, `false` if the todo or todo list doesn't exist. The id arguments must both be numeric. */
